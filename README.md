...
tests/pytest 디렉토리 내의 테스트 코드를 실행합니다.
...
# 디버그 모드 실행
# -vv : 상세 정보(Verbose) 출력
# -s : 표준 출력(print 문 등) 캡처 안 함 (로그 확인용)
...
## B. 트러블 슈팅 (Troubleshooting)

### 1. How to install pymssql on macOS (Using SQL server)

macOS 환경에서 pymssql 설치 시 아키텍처 호환성 문제가 발생할 경우 다음과 같이 FreeTDS와 OpenSSL을 설정해야 합니다.
